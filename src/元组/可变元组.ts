// 可变元组
let [username, no, ...rest]: [string, number, ...any[]] = ['fet', 123, 'man', '190cm', 'senior']
console.log('rest', rest)

// 给每个类型添加标签
let [user, id, ...rest1]: [user_: string, id_: number, ...rest: any[]] = ['fet', 123, 'man', '190cm', 'senior']
console.log('rest1', rest1)

// 限制元组的前两个和最后一个类型，其余可变
let [user1, id1, ...rest2]: [user_: string, id_: number, ...rest: any[], last_: string] = ['fet', 123, 'man', '190cm', 'senior', 'lastone']
...
type Res = UnionToTurple<never> // ['a', 'b', 'c']

type UnionToTurple<T, U = T> = T extends never ? [1] :  U extends U ? [U, UnionToTurple<never>] : []
