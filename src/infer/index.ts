...
// or 
type Concat2<T, U> = T extends [...infer R] ? U extends [...infer K] ? [...R, ...K] : R : never
...
type ReplaceAll<T extends string, From extends string, To extends string> = 
    From extends ''
    ? T
    : T extends `${infer L}${From}${infer R}` ? ReplaceAll<`${L}${To}${R}`, From, To> : T
...
type Split<T extends string> = T extends `${infer U}.${infer Rest}` ? U | Split<Rest> : T
